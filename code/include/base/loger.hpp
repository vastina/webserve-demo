...
constexpr unsigned int FILENAME_LENGTH_MAX = 256;  // the max filename length
constexpr unsigned int MAX_LOG_NUM = 999;

constexpr int LOG_STATUS_OK  = 0;
constexpr int LOG_STATUS_ERR = 1;
constexpr int LOG_MAX_LINE   = 100000;
  ...
  if (vastina::g_log) {                                                     \
    ...
    if (p) {                                                               \
      ...
      snprintf(sz_head,                                                    \
          ...
          (int32_t)getpid(),                                               \
          gettid(),                                                        \
          vastina::g_log->context_id());                                    \
      vastina::g_log->rotate(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday); \
    }                                                                      \
    snprintf(prefix,                                                       \
        sizeof(prefix),                                                    \
        "[%s %s %s@%s:%u] >> ",                                            \
        sz_head,                                                           \
        (vastina::g_log)->prefix_msg(level),                                \
        __FUNCTION__,                                                      \
        __FILE_NAME__,                                                     \
        (int32_t)__LINE__);                                                \
  }
...
#define LOG_DEFAULT(fmt, ...) LOG_OUTPUT(vastina::g_log->get_log_level(), fmt, ##__VA_ARGS__)
#define LOG_PANIC(fmt, ...) LOG_OUTPUT(vastina::LOG_LEVEL_PANIC, fmt, ##__VA_ARGS__)
#define LOG_ERROR(fmt, ...) LOG_OUTPUT(vastina::LOG_LEVEL_ERR, fmt, ##__VA_ARGS__)
#define LOG_WARN(fmt, ...) LOG_OUTPUT(vastina::LOG_LEVEL_WARN, fmt, ##__VA_ARGS__)
#define LOG_INFO(fmt, ...) LOG_OUTPUT(vastina::LOG_LEVEL_INFO, fmt, ##__VA_ARGS__)
#define LOG_DEBUG(fmt, ...) LOG_OUTPUT(vastina::LOG_LEVEL_DEBUG, fmt, ##__VA_ARGS__)
#define LOG_TRACE(fmt, ...) LOG_OUTPUT(vastina::LOG_LEVEL_TRACE, fmt, ##__VA_ARGS__)
